...
export class HeaderComponent {
  ...
  public FlagsForSLTP:any;
  constructor(
    private eRef: ElementRef,
    private headertoFooterSer: HeaderToFooterService,
    ...
    this.connectionStatus =
      localStorage.getItem('status') === 'Connect' ? 'Disconnect' : 'Connect';
    ...
    this.share.orderFlags$.subscribe((flags: number[]) => {
      this.FlagsForSLTP = flags;
      console.log("flag",this.FlagsForSLTP)
    ...
    this.accountActiveList = this.accountList.filter(
      (list: any) => list.account === Number(localStorage.getItem('loginId'))
    );
  ...
  onLotInputChange() {
    ...
    let cleanValue = this.cleanVolumeInput(this.inputLotValue);
    ...
    if (numValue < this.minLot) {
      this.inputLotValue = this.minLot;
    } else if (numValue > this.MaxLot) {
      this.inputLotValue = this.MaxLot;
    } else {
      this.inputLotValue = numValue;
    }
  }

  // Clean volume input to allow only numbers and one dot
  cleanVolumeInput(value: any): string {
  ...
  // Volume input keypress handler
  onVolumeKeyPress(event: KeyboardEvent): boolean {
    ...
    if (dotIndex !== -1) {
      ...
      if (decimalPart.length >= 2 && input.selectionStart! > dotIndex) {
        event.preventDefault();
        return false;
      }
    }
  ...
  onVolumeInput(event: any): void {
    ...
    // Clean the input
    const cleanValue = this.cleanVolumeInput(value);

    // Update the input field
    if (cleanValue !== value) {
      input.value = cleanValue;
      this.inputLotValue = cleanValue;
    }
  }

  // Volume blur handler (when clicking outside)
  onVolumeBlur(): void {
    let value = this.inputLotValue;

    // Clean the input
    const cleanValue = this.cleanVolumeInput(value);
    let numValue = parseFloat(cleanValue);

    // If invalid, empty, or just a dot, reset to minimum
    if (
      isNaN(numValue) ||
      cleanValue === '' ||
      cleanValue === '.' ||
  ...
  addVol() {
    // Ensure inputLotValue is a valid number
    let currentValue = parseFloat(this.inputLotValue) || this.minLot;

    const newVal = parseFloat((currentValue + this.stepVol).toFixed(2));
  ...
  private getInitialForDialogDisplay(symbol: string) {
    ...
    this.subscriptions.add(
      this.api.GET_SYMBOL_PROP(obj).subscribe({
        // Ensure api service and method name are correct
        next: (res: any) => {
          ...
          console.log(
            'GET_SYMBOL_PROP response for dialog display:',
            this.inputLotValue,
            res
          );
          console.log(
            `Fetched initial data for dialog display (${symbol}):`,
            res
          );
          if (
            this.dialogDisplayData &&
            this.dialogDisplayData.oSymbolConfig?.Symbol === symbol
          ) {
            ...
            if (res && typeof res === 'object' && !Array.isArray(res)) {
              if (res.oInitial) {
                ...
                res.hasOwnProperty('Bid') ||
                res.hasOwnProperty('Ask')
              ) {
              ...
              if (firstItem?.oInitial) {
                ...
                firstItem &&
                (firstItem.hasOwnProperty('Bid') ||
                  firstItem.hasOwnProperty('Ask'))
              ) {
            ...
            if (initialDataToUpdate) {
              this.dialogDisplayData.oInitial = {
                ...this.dialogDisplayData.oInitial,
                ...initialDataToUpdate,
              };
              ...
              const newPrice =
                initialDataToUpdate.Ask ??
                initialDataToUpdate.Bid ??
                this.currentPri ??
                null;
              this.currentPri = newPrice;
              console.log(
                `Updated currentPri based on local selection (${symbol}):`,
                this.currentPri
              );
              ...
              if (
                ...
                currentFormPrice === undefined ||
                currentFormPrice === '' ||
                (this.currentPri !== null &&
                  currentFormPrice == this.currentPri)
              ) {
                // Use == for potential string/number comparison
                this.orderFrom.patchValue({ price: this.currentPri });
                console.log(
                  'Patched order form price with currentPri from local symbol fetch.'
                );
              }
            } else {
              console.warn(
                'Could not extract oInitial data from response for:',
                symbol,
                res
              );
            ...
            console.log(
              'Ignoring update for stale/non-matching symbol:',
              symbol
            );
          }
        },
        error: (err: any) => {
          console.error(
            'Error fetching initial data for dialog display symbol:',
            symbol,
            err
          );
          if (
  ...
  changeSymbolData(val: any) {
    this.share.allMarketLiveData$.subscribe((res: any) => {
      ...
      this.data = res.filter(
        (item: any) => item?.oSymbolConfig?.Symbol === val
      );
    ...
    this.subscriptions.add(
      this.api.GET_SYMBOL_PROP(obj).subscribe({
        next: (res: any) => {
          ...
          // Format existing SL/TP values according to new precision
          this.formatExistingSlTpValues();
        ...
        error: (err: any) => {
          console.error(
            'Error in GET_SYMBOL_PROP for main data symbol:',
            val,
            err
          );
  ...
  formatTimeString(timeString: string): string {
    ...
    const valueData = `${hours.padStart(2, '0')}:${minutes.padStart(
      2,
      '0'
  ...
  decimalValidator(control: AbstractControl): { [key: string]: any } | null {
    ...
    if (parts.length === 2 && parts[1].length > precision) {
      return { invalidDecimal: true, maxPrecision: precision };
    }
  ...
  // Method to format existing SL/TP values when precision changes
  formatExistingSlTpValues() {
    if (!this.orderFrom) return;

    const precision = this.pricePrecision || 2;

    // Format Stop Loss
    const slControl = this.orderFrom.get('stopLoss');
    if (slControl?.value && slControl.value !== '') {
      const slValue = parseFloat(slControl.value);
      if (!isNaN(slValue)) {
        slControl.setValue(slValue.toFixed(precision), { emitEvent: false });
      }
    }

    // Format Take Profit
    const tpControl = this.orderFrom.get('takeProfit');
    if (tpControl?.value && tpControl.value !== '') {
      const tpValue = parseFloat(tpControl.value);
      if (!isNaN(tpValue)) {
        tpControl.setValue(tpValue.toFixed(precision), { emitEvent: false });
      }
    }

    // Format Stop Limit Price
    const slpControl = this.orderFrom.get('stopLimitPrice');
    if (slpControl?.value && slpControl.value !== '') {
      const slpValue = parseFloat(slpControl.value);
      if (!isNaN(slpValue)) {
        slpControl.setValue(slpValue.toFixed(precision), { emitEvent: false });
      }
    }

    // Format Price
    const priceControl = this.orderFrom.get('price');
    if (priceControl?.value && priceControl.value !== '') {
      const priceValue = parseFloat(priceControl.value);
      if (!isNaN(priceValue)) {
        priceControl.setValue(priceValue.toFixed(precision), {
          emitEvent: false,
        });
      }
    }
  ...
  // Helper method to validate and format decimal input for SL/TP
  validateAndFormatDecimal(value: string): string {
  ...
  // Comment validator to prevent whitespace-only comments
  commentValidator(control: AbstractControl): { [key: string]: any } | null {
    if (!control.value) return null; // Allow empty comments
  ...
  // Handle comment input blur to trim whitespace
  onCommentBlur(): void {
    const commentControl = this.orderFrom.get('comment');
    if (commentControl?.value) {
      const trimmedValue = commentControl.value.trim();
      if (trimmedValue === '') {
        commentControl.setValue('', { emitEvent: false });
      } else {
        commentControl.setValue(trimmedValue, { emitEvent: false });
      }
    }
  }
  ngOnInit() {
    ...
    this.orderFrom = this.fb.group({
      ...
      comment: ['', [this.commentValidator.bind(this)]], // Control for the Comment input with validation
  ...
  changePass() {
    ...
    this.api.MAKE_CHANGE_PASSWORD(payload).subscribe({
      next: (res: any) => {
  ...
  openXl3(content: any) {
    ...
    this.modref1 = this.modalService.open(content, {
      size: 'md modalone',
      centered: true,
    });
  }
  ...
  MAKE_NEW_ORDER_Market(val: any, price: any) {
    ...
    const symbolForOrder =
      this.data[0]?.oSymbolConfig?.Symbol || localStorage.getItem('changeSym');
    ...
    // Validate and clean volume before sending order
    let lotValue = this.inputLotValue || this.minLot;

    // Ensure lot value is within bounds and properly formatted
    if (lotValue < this.minLot) {
      lotValue = this.minLot;
    } else if (lotValue > this.MaxLot) {
      lotValue = this.MaxLot;
    }

    // Round to 2 decimal places
    lotValue = lotValue.toFixed(2);

    // Format SL and TP values according to precision before sending
    const precision = this.pricePrecision || 2;
    const slValue = this.orderFrom.value.stopLoss
      ? this.orderFrom.value.stopLoss
      : 0;
    const tpValue = this.orderFrom.get('takeProfit')?.value
      ? 
          this.orderFrom.get('takeProfit')?.value
        
      : 0;

    // Get and validate comment
    const commentValue = this.orderFrom.value.comment ? 
      this.orderFrom.value.comment.trim() : '';
  ...
  orderRes(byteArray: Uint8Array): any {
    ...
    if (MsgID == 301) {
      ...
      if (byteArray.length >= 120) {
        ...
        console.warn(
          `Payload too short for Balance/Margin read in MsgID 301: length = ${byteArray.length}`
        );
      }
  ...
  MAKE_NEW_ORDER(val: any) {
    ...
    const slValue = this.orderFrom.value.stopLoss
      ?(this.orderFrom.value.stopLoss)
      : 0;
    const tpValue = this.orderFrom.value.takeProfit
      ? 
          this.orderFrom.value.takeProfit
        
      : 0;
    const priceValue = this.orderFrom.value.price
      ? this.orderFrom.value.price
      : 0;
    const stopLimitValue = this.orderFrom.value.stopLimitPrice
      ? 
          this.orderFrom.value.stopLimitPrice
  ...
  addValue(val: any) {
    if (val == 1) {
      this.inputLotValue = (
        parseFloat(this.inputLotValue) +
        1 / Math.pow(10, this.countDecimalDigits(this.inputLotValue))
      ).toFixed(this.countDecimalDigits(this.inputLotValue));
    } else if (val == 2) {
      this.inputLotValue = (
        parseFloat(this.inputLotValue) +
        1 / Math.pow(10, this.countDecimalDigits(this.inputLotValuew))
      ).toFixed(this.countDecimalDigits(this.inputLotValue));
    }
  }
  ...
  addSll() {
    const slControl = this.orderFrom.get('stopLoss');
    const currentSL =
      parseFloat(slControl?.value) ||
      parseFloat(this.orderFrom.get('price')?.value) ||
      0;
    const step = this.getStepFromPrice();
    const precision = this.pricePrecision || 2;

    const newSL = (currentSL + step).toFixed(precision);
    this.inputSl = newSL;
    slControl?.setValue(newSL);
  }
  ...
  addTP() {
    const tpControl = this.orderFrom.get('takeProfit');
    const currentTP =
      parseFloat(tpControl?.value) ||
      parseFloat(this.orderFrom.get('price')?.value) ||
      0;
    const step = this.getStepFromPrice();
    const precision = this.pricePrecision || 2;

    const newTP = (currentTP + step).toFixed(precision);
    this.inputTPP = newTP;
    tpControl?.setValue(newTP);
  }
  ...
  addSll1() {
    ...
    const newSl =
      currentSl === 0
        ? parseFloat(this.orderFrom.get('price')?.value) + step
        : currentSl + step;

    slControl?.setValue(newSl.toFixed(precision));
  }
  ...
  addPricee(val: any) {
    ...
    const decimalPartLength =
      decimalIndex === -1 ? 0 : numberAsString.length - decimalIndex - 1;

    this.currentAddP = (
      parseFloat(val) +
      1 / Math.pow(10, decimalPartLength)
    ).toFixed(this.countDecimalDigits(val));
  ...
  subPricee(val: any) {
    ...
    let newSL = (
      parseFloat(val) -
      1 / Math.pow(10, this.countDecimalDigits(val))
    ).toFixed(this.countDecimalDigits(val));
  ...
  // }
  onDecimalBlur(field: 'price' | 'stopLoss' | 'takeProfit' | 'stopLimitPrice') {
    const control = this.orderFrom.get(field);
    if (!control) return;
    ...
    if (value.endsWith('.')) {
      value = value.slice(0, -1); // 12. → 12
    }

    // Parse and validate according to precision
    const numValue = parseFloat(value);
    if (!isNaN(numValue)) {
      const precision = this.pricePrecision || 2;

      // Check if the current value has more decimal places than allowed
      const parts = value.split('.');
      if (parts.length === 2 && parts[1].length > precision) {
        // Only format if it exceeds precision, otherwise keep as is
        const formattedValue = numValue.toFixed(precision);
        control.setValue(formattedValue, { emitEvent: false });
      } else {
        // Keep the original value if it's within precision limits
  ...
  onDecimalInputKeyPress(
    event: KeyboardEvent,
    field: 'price' | 'stopLoss' | 'takeProfit' | 'stopLimitPrice'
  ) {
    ...
    // Prevent exceeding decimal places for SL/TP fields specifically
    if (field === 'stopLoss' || field === 'takeProfit') {
      ...
      // For price and stopLimitPrice, use existing logic
      const parts = nextValue.split('.');
      if (parts.length === 2 && parts[1].length > precision) {
        event.preventDefault();
        return;
  ...
  isPlaceButtonDisabled(): boolean {
    ...
    if (orderType === 6) {
      const stopLimitPrice = parseFloat(
        this.orderFrom.get('stopLimitPrice')?.value || '0'
      );
      const lot = parseFloat(this.inputLotValue);

      if (!price || !stopLimitPrice || !lot || lot <= 0 || price <= marketPrice)
        return true;
  ...
  openXl2(content2: any) {
    ...
    if (initialSymbol) {
      ...
      if (
        this.dialogDisplayData &&
        (!this.dialogDisplayData.oInitial ||
          Object.keys(this.dialogDisplayData.oInitial).length === 0)
      ) {
    ...
    if (
      existingFormPrice === null ||
      existingFormPrice === undefined ||
      existingFormPrice === ''
    ) {
      if (this.currentPri !== null && this.currentPri !== undefined) {
        this.orderFrom.patchValue({ price: this.currentPri }); // Initialize lot size input if needed
        console.log(
          'Patched order form price with initialized currentPri:',
          this.currentPri
        );
      } else {
        console.log(
          'currentPri is null, order form price not patched automatically.'
        );
      }
    } else {
      console.log(
        'Order form price already has a value, not overwritten:',
        existingFormPrice
      );
    }
    ...
    if (
      initialFormPrice !== null &&
      initialFormPrice !== undefined &&
      initialFormPrice !== ''
    ) {
    ...
    this.priceControlSubscription = this.orderFrom
      .get('price')
      ?.valueChanges.subscribe((newPriceValue: any) => {
        console.log(
          'Price form control changed to:',
          newPriceValue,
          'Updating currentAddP.'
        );
  ...
  private resetModalData() {
    ...
    if (this.orderFrom) {
      this.orderFrom.patchValue({
        ...
        expirationType: 1, // Reset Expiration Type to default
  ...
  openDemoAccount() {
    if (this.openDemoAccountForm.valid) {
      ...
      this.global.OPEN_DEMO_ACCOUNT(obj).subscribe({
        next: (res: any) => {
          console.log('res', res);
        },
      });
  ...
  getDepositValue(value: string): string {
    const depositOptions = ['5000', '2000', '1000', '500', '300', '100'];
    return depositOptions[+value] || 'Unknown';
  }

  onLeverageChange() {
    this.openDemoAccountForm
      .get('leverage')
  ...
  getLeverageValue(value: string): string {
    const leverageOptions = ['500', '200', '100', '50', '30', '10'];
  ...
  getStatusActivity(val: any) {
    if (val.Update == 9 && val.State == 4 && val.MsgID == 300) {
      ...
      (val.Update == 4 && val.State == 4 && val.MsgID == 300) ||
      val.ERR_MSG == 'POSITION_NOT_EXISTS'
    ) {
  ...
  getSLUpdate(val: any) {
    ...
    const positionListData = JSON.parse(
      localStorage.getItem('positionListData') || '[]'
    );

    // Find the index of the item with the matching Ticket ID
    const index = positionListData.findIndex(
      (item: any) => item.Ticket === val.Ticket
    );
    ...
    if (index !== -1) {
      ...
      localStorage.setItem(
        'positionListData',
        JSON.stringify(positionListData)
      ); // Save updated array
  ...
  deletePosition(ticketId: any) {
    ...
    if (storedData) {
      ...
      const updatedData = positionListData.filter(
        (item: any) => item.Ticket == ticketId
      );
  ...
  deletePositions(ticketId: any) {
    ...
    if (storedData1) {
      ...
      const updatedData1 = positions.filter(
  ...
  getLotUpdate(val: any) {
    ...
    if (index !== -1) {
      ...
      if (
        Number(positionListData[index].Lot) == Number(val.Lot) ||
        LOT == 0 ||
        Number(positionListData[index].Lot) == 0
      ) {
  ...
  GET_OPENED1() {
    ...
    this.api.GET_USER_OPEN_POS(obj).subscribe({
      next: (res: any) => {
        // this.startInterval()
